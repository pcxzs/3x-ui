...
func TestResetAllTrafficsAnswersWithNoClients(t *testing.T) {
	...
	// This branch lands the admin back on the Bulk actions menu instead.
	if !strings.Contains(string(got[0].ReplyMarkup), `"bulk_preview extend"`) {
		t.Errorf("reply_markup = %s, want the bulk actions keyboard", got[0].ReplyMarkup)
	}
}
