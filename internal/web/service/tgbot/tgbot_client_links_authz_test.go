...
func newLinksCallbackTgbot(t *testing.T, email string) (*Tgbot, func(string) int) {
	...
	// This branch checks ownership against the clients table, not inbound JSON.
	if err := database.GetDB().Create(&model.ClientRecord{Email: email, SubID: "sub-owned", TgID: ownerTgID}).Error; err != nil {
