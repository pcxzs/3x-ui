...
// One daily notice the admin can switch off. Read and write stay together so a
// new notice cannot be listed in the menu without being wired to its setting.
...
func notificationByCallback(callback string) (dailyNotification, bool) {
	for _, notification := range dailyNotifications {
		if notification.callback == callback {
			return notification, true
		}
	}
	return dailyNotification{}, false
}
...
func (t *Tgbot) notificationToggleLabel(notification dailyNotification) string {
...
func (t *Tgbot) notificationsKeyboard() *telego.InlineKeyboardMarkup {
	rows := make([][]telego.InlineKeyboardButton, 0, len(dailyNotifications)+1)
	for _, notification := range dailyNotifications {
...
func (t *Tgbot) notificationsMenu(chatId int64) {
	t.SendMsgToTgbot(chatId, t.I18nBot("tgbot.messages.dailyNotifications"), t.notificationsKeyboard())
}
